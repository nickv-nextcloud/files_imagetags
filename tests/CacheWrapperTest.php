...
use OCA\FilesImageTags\CacheWrapper;
use Test\TestCase;
...
class CacheWrapperTest extends TestCase {
	...
	protected $storage;
	/** @var \OCA\FilesImageTags\Worker|\PHPUnit_Framework_MockObject_MockObject */
	protected $worker;
	...
	protected function setUp() {
		...
		$this->storage = $this->getMockBuilder('OCP\Files\Storage\IStorage')
			->getMock();
		$this->worker = $this->getMockBuilder('OCA\FilesImageTags\Worker')
	...
	public function testInsert($mountPoint, $path, array $data, $fileId, $checkOperations) {
		$test = new CacheWrapper($this->cache, $this->storage, $this->worker, $mountPoint);
		...
		$this->worker->expects($checkOperations ? $this->once() : $this->never())
			->method('readAndAssignTags')
